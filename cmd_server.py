...
class Selector:
    ...
    def shuffle(self):
        ...
        while index < len(self.candies):
            if self.weights[index - 1] <= result < self.weights[index]:
                break
            index += index

        return self.candies[index-1]
...
class reply_server:
    ...
    def set_cmd_type(self, cmd, arg):
        ...
        if self.checkout(cmd, super_user):
            self.db.set_cmd_type(cmd, int(arg.strip()))
    ...
    def save_pic(self, pic: PicObj, tag="", reply=""):
        if pic.Url:
            try:
                ...
                new_reply_id = self.cmd_info.sequences[CMD_TYPE.PIC] + 1
                self.db.add_reply(self.cmd_info.cmd_id, CMD_TYPE.PIC, new_reply_id, tag=tag, md5=pic.Md5, file_type=img_type, reply=reply, user_id=pic.user)
